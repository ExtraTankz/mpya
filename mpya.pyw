...
def get_weather(city, state, zipcode):
    # send the GET request and catch any exceptions that may occur
    try:
        response = requests.get(f"{API_URL}?location={city},{state},{zipcode}&units=imperial&apikey={API_KEY}")
        response.raise_for_status()
...
def get_location():
    ...
    layout = [
        [sg.Text(
            ...
            pad=pad)],
        [sg.Text(
            "Note: For sunrise/sunset data fill in City.", 
            font=font, 
            pad=pad)],
        [sg.HSeparator()],
        [sg.Frame(
    ...
    return values.get("city", ""), values.get("state", ""), values.get("zip", ""), selected_radar

def retrieve_radar_image(selected_radar):
    url = "https://radar.weather.gov/ridge/standard/CONUS_loop.gif" if selected_radar == "National Weather Service" else "https://s.w-x.co/staticmaps/wu/wu/wxtype1200_cur/conus/animate.png"
...
try:
    ...
    layout = [
        [sg.Column([[sg.Text(
            ...
            font=font, text_color="white", pad=pad, size=(None, 1),
        ...
        [sg.Column([[sg.Text(
            ...
            justification="center")]], element_justification="center", expand_x=True)],
        [sg.Column([[sg.Text(
            weather_str, font=font, pad=pad, size=(None, 3),
            justification="center")]], element_justification="center", expand_x=True)],
        [sg.Frame("", [[sg.Image(
            filename="picture.gif", key="-IMAGE-", pad=pad,
            background_color="white")]], border_width=0, pad=pad)],
        [sg.Column([[sg.Button(
            "OK", button_color=button_color, font=font, pad=pad)]],
    ...
    error_message = "API Limit reached, Check Tomorrow.io!" if "429" in str(err) else f"Error: {err}"
